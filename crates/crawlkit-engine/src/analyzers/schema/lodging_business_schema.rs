        ...
        for sd in &ctx.page.structured_data {
                ...
                .is_empty()
            {
                findings.push(Finding {
                    ...
                    description:
                        "A LodgingBusiness structured data block is missing the \"name\" property."
                            .to_string(),
                    url: url.clone(),
...
mod tests {
    ...
    fn test_lodge_missing_name() {
        ...
        page.structured_data = vec![StructuredData {
            ...
            r#type: Some("LodgingBusiness".to_string()),
            data: serde_json::json!({"@type": "LodgingBusiness"}),
        }];
        assert!(LodgingBusinessSchemaValidator::new()
            .analyze(&make_ctx(&page))
            .iter()
            .any(|f| f.code == "LODGE001"));
    }
    ...
    fn test_lodge_missing_address() {
        ...
        page.structured_data = vec![StructuredData {
            context: Some("https://schema.org".to_string()),
            r#type: Some("LodgingBusiness".to_string()),
            data: serde_json::json!({"@type": "LodgingBusiness", "name": "Hotel"}),
        }];
        assert!(LodgingBusinessSchemaValidator::new()
            .analyze(&make_ctx(&page))
            .iter()
            .any(|f| f.code == "LODGE002"));
    }
    ...
    fn test_lodge_missing_telephone() {
        ...
        page.structured_data = vec![StructuredData {
            context: Some("https://schema.org".to_string()),
            r#type: Some("LodgingBusiness".to_string()),
            data: serde_json::json!({"@type": "LodgingBusiness", "name": "Hotel", "address": {"@type": "PostalAddress"}}),
        }];
        assert!(LodgingBusinessSchemaValidator::new()
            .analyze(&make_ctx(&page))
            .iter()
            .any(|f| f.code == "LODGE003"));
    }
    ...
    fn test_lodge_valid() {
        ...
        page.structured_data = vec![StructuredData {
            context: Some("https://schema.org".to_string()),
            r#type: Some("LodgingBusiness".to_string()),
            data: serde_json::json!({"@type": "LodgingBusiness", "name": "Hotel", "address": {"@type": "PostalAddress"}, "telephone": "+1-555-0100"}),
        }];
        assert!(LodgingBusinessSchemaValidator::new()
            .analyze(&make_ctx(&page))
            .is_empty());
    ...
    fn test_lodge_all_issues() {
        ...
        assert_eq!(
            LodgingBusinessSchemaValidator::new()
                .analyze(&make_ctx(&page))
                .len(),
            3
        );
    ...
    fn test_lodge_empty_name() {
        ...
        page.structured_data = vec![StructuredData {
            context: Some("https://schema.org".to_string()),
            r#type: Some("LodgingBusiness".to_string()),
            data: serde_json::json!({"@type": "LodgingBusiness", "name": ""}),
        }];
        assert!(LodgingBusinessSchemaValidator::new()
            .analyze(&make_ctx(&page))
            .iter()
