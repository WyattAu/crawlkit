...
impl Analyzer for PerformingArtsSeriesSchemaValidator {
    fn name(&self) -> &str { "performing-arts-series-schema" }

    fn analyze(&self, ctx: &AnalysisContext) -> Vec<Finding> {
        let mut findings = Vec::new();
        let url = &ctx.page.url;
        for sd in &ctx.page.structured_data {
            if sd.r#type.as_deref() != Some("PerformingArtsSeries") { continue; }
            let data = &sd.data;
...
mod tests {
    ...
    #[test] fn test_perform_missing_name() { let mut p = make_page("https://example.com"); p.structured_data = vec![StructuredData { context: Some("https://schema.org".into()), r#type: Some("PerformingArtsSeries".into()), data: serde_json::json!({"@type":"PerformingArtsSeries"}) }]; assert!(PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)).iter().any(|f| f.code == "PERFORMARTS001")); }
    #[test] fn test_perform_missing_org() { let mut p = make_page("https://example.com"); p.structured_data = vec![StructuredData { context: Some("https://schema.org".into()), r#type: Some("PerformingArtsSeries".into()), data: serde_json::json!({"@type":"PerformingArtsSeries","name":"Concert Series"}) }]; assert!(PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)).iter().any(|f| f.code == "PERFORMARTS002")); }
    #[test] fn test_perform_missing_events() { let mut p = make_page("https://example.com"); p.structured_data = vec![StructuredData { context: Some("https://schema.org".into()), r#type: Some("PerformingArtsSeries".into()), data: serde_json::json!({"@type":"PerformingArtsSeries","name":"Concert Series","organizer":{"@type":"Organization","name":"Org"}}) }]; assert!(PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)).iter().any(|f| f.code == "PERFORMARTS003")); }
    #[test] fn test_perform_valid() { let mut p = make_page("https://example.com"); p.structured_data = vec![StructuredData { context: Some("https://schema.org".into()), r#type: Some("PerformingArtsSeries".into()), data: serde_json::json!({"@type":"PerformingArtsSeries","name":"Concert Series","organizer":{"@type":"Organization"},"event":{"@type":"Event"}}) }]; assert!(PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)).is_empty()); }
    #[test] fn test_perform_non_perform_ignored() { let mut p = make_page("https://example.com"); p.structured_data = vec![StructuredData { context: Some("https://schema.org".into()), r#type: Some("Product".into()), data: serde_json::json!({"@type":"Product"}) }]; assert!(PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)).is_empty()); }
    #[test] fn test_perform_no_data() { let p = make_page("https://example.com"); assert!(PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)).is_empty()); }
    #[test] fn test_perform_all_issues() { let mut p = make_page("https://example.com"); p.structured_data = vec![StructuredData { context: Some("https://schema.org".into()), r#type: Some("PerformingArtsSeries".into()), data: serde_json::json!({"@type":"PerformingArtsSeries"}) }]; assert_eq!(PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)).len(), 3); }
    #[test] fn test_perform_empty_name() { let mut p = make_page("https://example.com"); p.structured_data = vec![StructuredData { context: Some("https://schema.org".into()), r#type: Some("PerformingArtsSeries".into()), data: serde_json::json!({"@type":"PerformingArtsSeries","name":""}) }]; assert!(PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)).iter().any(|f| f.code == "PERFORMARTS001")); }
    #[test] fn test_perform_with_performer() { let mut p = make_page("https://example.com"); p.structured_data = vec![StructuredData { context: Some("https://schema.org".into()), r#type: Some("PerformingArtsSeries".into()), data: serde_json::json!({"@type":"PerformingArtsSeries","name":"Series","performer":{"@type":"Person","name":"Artist"},"subEvent":{"@type":"Event"}}) }]; assert!(PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)).is_empty()); }
    #[test] fn test_perform_name_only_two_issues() { let mut p = make_page("https://example.com"); p.structured_data = vec![StructuredData { context: Some("https://schema.org".into()), r#type: Some("PerformingArtsSeries".into()), data: serde_json::json!({"@type":"PerformingArtsSeries","name":"Series"}) }]; let f = PerformingArtsSeriesSchemaValidator::new().analyze(&make_ctx(&p)); assert!(f.iter().any(|f| f.code == "PERFORMARTS002")); assert!(f.iter().any(|f| f.code == "PERFORMARTS003")); }
}
