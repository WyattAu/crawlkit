...
impl Analyzer for EducationalOrganizationSchemaValidator {
    ...
    fn analyze(&self, ctx: &AnalysisContext) -> Vec<Finding> {
        ...
        for sd in &ctx.page.structured_data {
            if sd.r#type.as_deref() != Some("EducationalOrganization") {
                continue;
            }
            let data = &sd.data;
            if data
...
mod tests {
    ...
    fn test_edu_missing_name() {
        ...
        p.structured_data = vec![StructuredData {
            ...
            data: serde_json::json!({"@type":"EducationalOrganization"}),
        }];
        assert!(EducationalOrganizationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
            .any(|f| f.code == "EDUORG001"));
    }
    #[test]
    fn test_edu_missing_address() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("EducationalOrganization".into()),
            data: serde_json::json!({"@type":"EducationalOrganization","name":"MIT"}),
        }];
        assert!(EducationalOrganizationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
            .any(|f| f.code == "EDUORG002"));
    }
    #[test]
    fn test_edu_missing_url() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("EducationalOrganization".into()),
            data: serde_json::json!({"@type":"EducationalOrganization","name":"MIT","address":{"@type":"PostalAddress"}}),
        }];
        assert!(EducationalOrganizationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
            .any(|f| f.code == "EDUORG003"));
    }
    #[test]
    fn test_edu_valid() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("EducationalOrganization".into()),
            data: serde_json::json!({"@type":"EducationalOrganization","name":"MIT","address":{"@type":"PostalAddress"},"url":"https://mit.edu"}),
        }];
        assert!(EducationalOrganizationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .is_empty());
    ...
    fn test_edu_all_issues() {
        ...
        assert_eq!(
            EducationalOrganizationSchemaValidator::new()
                .analyze(&make_ctx(&p))
                .len(),
            3
        );
    ...
    #[test]
    fn test_edu_empty_name() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("EducationalOrganization".into()),
            data: serde_json::json!({"@type":"EducationalOrganization","name":""}),
        }];
        assert!(EducationalOrganizationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
    ...
    #[test]
    fn test_edu_name_only_two_issues() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("EducationalOrganization".into()),
        ...
        let f = EducationalOrganizationSchemaValidator::new().analyze(&make_ctx(&p));
        assert!(f.iter().any(|f| f.code == "EDUORG002"));
        assert!(f.iter().any(|f| f.code == "EDUORG003"));
    }
    #[test]
    fn test_edu_name_and_address_only() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("EducationalOrganization".into()),
        ...
        let f = EducationalOrganizationSchemaValidator::new().analyze(&make_ctx(&p));
        assert!(f.iter().any(|f| f.code == "EDUORG003"));
        assert!(!f.iter().any(|f| f.code == "EDUORG001"));
        assert!(!f.iter().any(|f| f.code == "EDUORG002"));
    }
}
