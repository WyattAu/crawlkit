...
impl Analyzer for TouristAttractionSchemaValidator {
    ...
    fn analyze(&self, ctx: &AnalysisContext) -> Vec<Finding> {
        ...
        for sd in &ctx.page.structured_data {
            if sd.r#type.as_deref() != Some("TouristAttraction") {
                continue;
            }
            let data = &sd.data;
            if data
...
mod tests {
    ...
    fn test_tourist_missing_name() {
        ...
        p.structured_data = vec![StructuredData {
            ...
            data: serde_json::json!({"@type":"TouristAttraction"}),
        }];
        assert!(TouristAttractionSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
            .any(|f| f.code == "TOURIST001"));
    }
    #[test]
    fn test_tourist_missing_location() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristAttraction".into()),
            data: serde_json::json!({"@type":"TouristAttraction","name":"Beach"}),
        }];
        assert!(TouristAttractionSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
            .any(|f| f.code == "TOURIST002"));
    }
    #[test]
    fn test_tourist_missing_type() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristAttraction".into()),
            data: serde_json::json!({"@type":"TouristAttraction","name":"Beach","address":{"@type":"PostalAddress"}}),
        }];
        assert!(TouristAttractionSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
            .any(|f| f.code == "TOURIST003"));
    }
    #[test]
    fn test_tourist_valid() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristAttraction".into()),
            data: serde_json::json!({"@type":"TouristAttraction","name":"Beach","address":{"@type":"PostalAddress"},"touristType":"Families"}),
        }];
        assert!(TouristAttractionSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .is_empty());
    ...
    #[test]
    fn test_tourist_non_tourist_ignored() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
    ...
    fn test_tourist_all_issues() {
        ...
        assert_eq!(
            TouristAttractionSchemaValidator::new()
                .analyze(&make_ctx(&p))
                .len(),
            3
        );
    ...
    #[test]
    fn test_tourist_empty_name() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristAttraction".into()),
            data: serde_json::json!({"@type":"TouristAttraction","name":""}),
        }];
        assert!(TouristAttractionSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
    ...
    #[test]
    fn test_tourist_with_geo() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristAttraction".into()),
            data: serde_json::json!({"@type":"TouristAttraction","name":"Beach","geo":{"@type":"GeoCoordinates"},"touristType":"Backpackers"}),
        }];
        assert!(TouristAttractionSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .is_empty());
    ...
    #[test]
    fn test_tourist_name_only_two_issues() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristAttraction".into()),
        ...
        let f = TouristAttractionSchemaValidator::new().analyze(&make_ctx(&p));
        assert!(f.iter().any(|f| f.code == "TOURIST002"));
        assert!(f.iter().any(|f| f.code == "TOURIST003"));
    }
}
