...
impl Analyzer for TouristDestinationSchemaValidator {
    ...
    fn analyze(&self, ctx: &AnalysisContext) -> Vec<Finding> {
        ...
        for sd in &ctx.page.structured_data {
            if sd.r#type.as_deref() != Some("TouristDestination") {
                continue;
            }
            let data = &sd.data;
            if data
...
mod tests {
    ...
    fn test_tdest_missing_name() {
        ...
        p.structured_data = vec![StructuredData {
            ...
            data: serde_json::json!({"@type":"TouristDestination"}),
        }];
        assert!(TouristDestinationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
            .any(|f| f.code == "TOURISTDEST001"));
    }
    #[test]
    fn test_tdest_missing_location() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristDestination".into()),
            data: serde_json::json!({"@type":"TouristDestination","name":"Paris"}),
        }];
        assert!(TouristDestinationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
            .any(|f| f.code == "TOURISTDEST002"));
    }
    #[test]
    fn test_tdest_missing_type() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristDestination".into()),
            data: serde_json::json!({"@type":"TouristDestination","name":"Paris","address":{"@type":"PostalAddress"}}),
        }];
        assert!(TouristDestinationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
            .any(|f| f.code == "TOURISTDEST003"));
    }
    #[test]
    fn test_tdest_valid() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristDestination".into()),
            data: serde_json::json!({"@type":"TouristDestination","name":"Paris","address":{"@type":"PostalAddress"},"touristType":"Couples"}),
        }];
        assert!(TouristDestinationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .is_empty());
    ...
    #[test]
    fn test_tdest_non_dest_ignored() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
    ...
    fn test_tdest_all_issues() {
        ...
        assert_eq!(
            TouristDestinationSchemaValidator::new()
                .analyze(&make_ctx(&p))
                .len(),
            3
        );
    ...
    #[test]
    fn test_tdest_empty_name() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristDestination".into()),
            data: serde_json::json!({"@type":"TouristDestination","name":""}),
        }];
        assert!(TouristDestinationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .iter()
    ...
    #[test]
    fn test_tdest_with_geo() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristDestination".into()),
            data: serde_json::json!({"@type":"TouristDestination","name":"Paris","geo":{"@type":"GeoCoordinates"},"touristType":"Families"}),
        }];
        assert!(TouristDestinationSchemaValidator::new()
            .analyze(&make_ctx(&p))
            .is_empty());
    ...
    #[test]
    fn test_tdest_name_only_two_issues() {
        let mut p = make_page("https://example.com");
        p.structured_data = vec![StructuredData {
            context: Some("https://schema.org".into()),
            r#type: Some("TouristDestination".into()),
        ...
        let f = TouristDestinationSchemaValidator::new().analyze(&make_ctx(&p));
        assert!(f.iter().any(|f| f.code == "TOURISTDEST002"));
        assert!(f.iter().any(|f| f.code == "TOURISTDEST003"));
    }
}
