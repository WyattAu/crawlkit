...
//! re-benching — so the benchmarks workflow runs it as a cheap second
//! gate right after the bench run.
//!
...
const USAGE: &str =
    "usage: crawlkit-budget-check --criterion-dir <dir> --budgets <percentile-budgets.toml>";
...
fn run(args: &Args) -> Result<(), GateError> {
    ...
    report.ensure_pass()?;
    println!("All percentile budgets within budget.");
    Ok(())
}
...
fn main() -> ExitCode {
    match parse_args(std::env::args().skip(1)).and_then(|args| run(&args)) {
        Ok(()) => ExitCode::SUCCESS,
        Err(err) => {
            eprintln!("crawlkit-budget-check: {err}");
            ExitCode::FAILURE
        }
    }
}
...
mod tests {
    ...
    fn committed_budgets_parse_into_percentile_kit_schema() {
        ...
        for entry in budgets {
            let budget: percentile_kit::Budget = entry.try_into().expect("entry matches schema");
    ...
    /// Every budgeted metric names a real bench in
    /// `crates/crawlkit-engine/benches/crawlkit_benchmarks.rs` — a
    /// renamed bench would otherwise hard-fail the CI gate with a
    /// missing-estimates error.
    #[test]
    fn every_budgeted_metric_exists_in_the_bench_suite() {
        let bench_source =
            include_str!("../../../crates/crawlkit-engine/benches/crawlkit_benchmarks.rs");
        let raw = include_str!("../../../percentile-budgets.toml");
        let file: toml::Value = toml::from_str(raw).expect("percentile-budgets.toml parses");
        let budgets = file
            .get("budget")
        ...
        for entry in budgets {
            let budget: percentile_kit::Budget = entry.try_into().expect("entry matches schema");
            // Criterion's output dir is `<group>/<bench-id>`; both parts
            // appear verbatim in the bench source.
            for part in budget.metric.split('/') {
                assert!(
                    bench_source.contains(&format!("\"{part}\"")),
                    "metric part `{part}` of `{}` not found in crawlkit_benchmarks.rs",
                    budget.metric
                );
            }
        }
    ...
    fn gate_passes_at_budget_and_fails_on_p99_breach() {
        ...
        // past the 25% allowance and fail the gate.
        std::fs::write(
            &budgets_path,
            "[[budget]]\nmetric = \"ci_gate_probe\"\np50 = 90.0\np99 = 5.0\nmax_regression_pct = 25.0\n",
        )
