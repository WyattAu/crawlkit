...
mod tests {
    ...
    fn test_sports_missing_name() {
        ...
        page.structured_data = vec![StructuredData { context: Some("https://schema.org".to_string()), r#type: Some("SportsActivityLocation".to_string()), data: serde_json::json!({"@type": "SportsActivityLocation"}) }];
        assert!(SportsActivityLocationSchemaValidator::new().analyze(&make_ctx(&page)).iter().any(|f| f.code == "SPORTS001"));
    }

    #[test]
    fn test_sports_missing_address() {
        let mut page = make_page("https://example.com");
        page.structured_data = vec![StructuredData { context: Some("https://schema.org".to_string()), r#type: Some("SportsActivityLocation".to_string()), data: serde_json::json!({"@type": "SportsActivityLocation", "name": "Gym"}) }];
        assert!(SportsActivityLocationSchemaValidator::new().analyze(&make_ctx(&page)).iter().any(|f| f.code == "SPORTS002"));
    }

    #[test]
    fn test_sports_valid() {
        let mut page = make_page("https://example.com");
        page.structured_data = vec![StructuredData { context: Some("https://schema.org".to_string()), r#type: Some("SportsActivityLocation".to_string()), data: serde_json::json!({"@type": "SportsActivityLocation", "name": "Gym", "address": {"@type": "PostalAddress"}}) }];
        assert!(SportsActivityLocationSchemaValidator::new().analyze(&make_ctx(&page)).is_empty());
    }
    ...
    fn test_sports_all_issues() {
        let mut page = make_page("https://example.com");
        page.structured_data = vec![StructuredData { context: Some("https://schema.org".to_string()), r#type: Some("SportsActivityLocation".to_string()), data: serde_json::json!({"@type": "SportsActivityLocation"}) }];
        assert_eq!(SportsActivityLocationSchemaValidator::new().analyze(&make_ctx(&page)).len(), 2);
    }

    #[test]
    fn test_sports_empty_name() {
        let mut page = make_page("https://example.com");
        page.structured_data = vec![StructuredData { context: Some("https://schema.org".to_string()), r#type: Some("SportsActivityLocation".to_string()), data: serde_json::json!({"@type": "SportsActivityLocation", "name": ""}) }];
        assert!(SportsActivityLocationSchemaValidator::new().analyze(&make_ctx(&page)).iter().any(|f| f.code == "SPORTS001"));
    }
    ...
    fn test_sports_multiple_locations() {
        ...
        page.structured_data = vec![
            StructuredData { context: Some("https://schema.org".to_string()), r#type: Some("SportsActivityLocation".to_string()), data: serde_json::json!({"@type": "SportsActivityLocation"}) },
            StructuredData { context: Some("https://schema.org".to_string()), r#type: Some("SportsActivityLocation".to_string()), data: serde_json::json!({"@type": "SportsActivityLocation", "name": "Pool"}) },
        ];
        let findings = SportsActivityLocationSchemaValidator::new().analyze(&make_ctx(&page));
    ...
    fn test_sports_valid_with_extra_fields() {
        let mut page = make_page("https://example.com");
        page.structured_data = vec![StructuredData { context: Some("https://schema.org".to_string()), r#type: Some("SportsActivityLocation".to_string()), data: serde_json::json!({"@type": "SportsActivityLocation", "name": "Gym", "address": {"@type": "PostalAddress"}, "telephone": "+1-555-0100", "priceRange": "$$"}) }];
        assert!(SportsActivityLocationSchemaValidator::new().analyze(&make_ctx(&page)).is_empty());
    }
}
